...
TxoutType SolverXXX(const CScript& scriptPubKey, std::vector<std::vector<unsigned char>>& vSolutionsRet)
{
    vSolutionsRet.clear();

    std::cout<<"Entered SolverXXX "<<std::endl;

    // Shortcut for pay-to-script-hash, which are more constrained than the other types:
    // it is always OP_HASH160 20 [20 byte hash] OP_EQUAL
    if (scriptPubKey.IsPayToScriptHash())
    {
        std::cout<<"Script is PayToScriptHash"<<std::endl;
        std::vector<unsigned char> hashBytes(scriptPubKey.begin()+2, scriptPubKey.begin()+22);
        vSolutionsRet.push_back(hashBytes);
        return TxoutType::SCRIPTHASH;
    }

    std::cout<<"Script is NOT PayToScriptHash"<<std::endl;

    int witnessversion;
    std::vector<unsigned char> witnessprogram;
    if (scriptPubKey.IsWitnessProgram(witnessversion, witnessprogram)) {
            std::cout<<"Script IsWitnessProgram "<<std::endl;
    ...
    std::cout<<"Script Not witnessProgram "<<std::endl;
    ...
    // script.
    if (scriptPubKey.size() >= 1 && scriptPubKey[0] == OP_RETURN && scriptPubKey.IsPushOnly(scriptPubKey.begin()+1)) {
        std::cout<<"TxoutType::NULL_DATA returned"<<std::endl;
        return TxoutType::NULL_DATA;
    }
        std::cout<<"TxoutType::NULL_DATA NOT returned"<<std::endl;

    std::vector<unsigned char> data;
    if (MatchPayToPubkey(scriptPubKey, data)) {
        std::cout<<"Script is PayToPubKey"<<std::endl;
        vSolutionsRet.push_back(std::move(data));
        return TxoutType::PUBKEY;
    }
        std::cout<<"Script is Not PayToPubKey"<<std::endl;

    if (MatchPayToPubkeyHash(scriptPubKey, data)) {
        std::cout<<"Script is PayToPubKeyHash"<<std::endl;

        vSolutionsRet.push_back(std::move(data));
        return TxoutType::PUBKEYHASH;
    }
        std::cout<<"Script is Not PayToPubKeyHash"<<std::endl;

    int required;
    std::vector<std::vector<unsigned char>> keys;
    if (MatchMultisig(scriptPubKey, required, keys)) {
        std::cout<<"Script is MultiSig"<<std::endl;
    ...
    vSolutionsRet.clear();
    std::cout<<"Script is NonStandarad"<<std::endl;
